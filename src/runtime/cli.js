...
import { configPathFromEnv } from '../core/config.js';
...
const BUILTINS = [
  ...
  },
  {
    id: 'help',
    cli: ['help'],
    summary: '显示帮助（可跟模块名或命令组，如 nx-kv help todo）',
  ...
  // render 让 `nx-kv version` 输出裸版本号（脚本里可 `V=$(nx-kv version)`），
  // 而 `--json` 仍走序列化，保持机器可读
...
function splitGlobals(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--config') { cfgPath = argv[++i]; continue; }
    if (a.startsWith('--config=')) { cfgPath = a.slice('--config='.length); continue; }
    rest.push(a);
  }
  return { rest, json, cfgPath };
}
...
function mapPositionals(positionals, argSpecs) {
  ...
  if (i < positionals.length) {
    throw badInput(`多余的参数: ${positionals.slice(i).join(' ')}（用法见 nx-kv help）`);
  }
  return out;
}
...
function assertKnownFlags(flags, cmd) {
  ...
  for (const k of Object.keys(flags)) {
    if (!known.has(k)) throw badInput(`未知参数 --${k}（用法见 nx-kv help）`);
  }
}
...
export function commandEntry(c) {
  return {
    ...
    command: 'nx-kv ' + cliPathsOf(c)[0].join(' '),
    usage: usageOf(c),
...
// 这条路径不能报错退出：`nx-kv help <随便什么>` 失败会让最需要帮助的人卡住。
function helpEntries(topic) {
  ...
  // 命令本身的 id 也认（如 `nx-kv help repo.add`）
...
function renderHelp(entries, topic) {
  const lines = [];
  if (!topic) {
    lines.push(`nx-kv · nx-kv v${VERSION}`);
    lines.push('KV 清单管理中枢：serve 驱动 CLI 与 Web，同源同构。');
    lines.push(`配置: ${configPathFromEnv()}    （环境变量 NX_KV_CONFIG 或 --config 覆盖）`);
    lines.push('');
  }
  ...
  lines.push('  --config  本次运行覆盖配置文件路径');
  return lines.join('\n');
}

// `nx-kv <命令> --help`：只打印这一条，而不是整个模块——
...
function printCommandHelp(cmd) {
  ...
  const aliases = cliPathsOf(cmd).slice(1).map((p) => 'nx-kv ' + p.join(' '));
...
async function cmdServe(ctx) {
  ...
  console.log('nx-kv · nx-kv v' + VERSION);
  console.log(`面板:   ${addr}`);
  console.log(`配置:   ${configPathFromEnv()}`);
  console.log('CLI:    nx-kv help（每个按钮都有对应命令，agent 可加 --json）');
...
export async function runCli(argv) {
  const { rest, json, cfgPath } = splitGlobals(argv);
  if (cfgPath) process.env.NX_KV_CONFIG = cfgPath;
  ...
  if (!hit) {
    ...
    console.error('运行 nx-kv help 查看用法');
    process.exitCode = 1;
    return;
  }
