// 内置 skill 包模块：把随包发布的 nx-kv 说明装到用户的 skills 目录。
...
export default {
  ...
  actions: [
    ...
    },
    {
      ...
      summary: '安装内置 skill 到 skills 目录（默认 nx-kv → ~/.claude/skills）',
      args: [{ name: 'name', required: false }],
      flags: { to: { type: 'string' }, force: { type: 'boolean' } },
      run: (ctx) =>
        service.installBundledSkill({ name: ctx.name || 'nx-kv', to: ctx.to, force: ctx.force }),
      render: (d) => {
