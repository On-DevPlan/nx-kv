// 内置 skill 包：随 nx-kv 一起发布的 assets/<name>/（SKILL.md + references/…）
// 负责列出与安装到用户/项目的 skills 目录，供 agent 一键获得 nx-kv 的使用说明。
...
// 需要用户决定是否覆盖，不是错误。
export async function installBundledSkill({ name = 'nx-kv', to, force } = {}) {
